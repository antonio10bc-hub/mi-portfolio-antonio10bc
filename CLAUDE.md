...
- `app/page.tsx` — home / landing, links into the two sections below. **It must fit in the viewport with no scrolling, footer included**: the root is `min-h-screen-safe` (never a fixed height with `overflow-hidden`, see Mobile), the hero and footer are `shrink-0`, the card grid is the only `flex-1` element, and every type size in it is a `clamp()` so it survives short viewports. Verified down to 320x568. If you add anything here, re-check that it still fits instead of assuming it does; a bad fit degrades into a scroll rather than clipping. The section counts on the cards come from `lib/photos.ts` and `lib/projects.ts`, so they stay true as content is added.
...
- `AnimatedBackground` — the drifting colour field. `variant="full"` (6 blobs) or `"minimal"` (3); `position="fixed"` (default) or `"absolute"`. Each blob is one `radial-gradient` built in JS from a core colour, a halo colour and a peak alpha (see Mobile for why it is not `filter: blur()`). Position and size stay as full literal Tailwind strings in the arrays so the scanner finds them; never concatenate those class names from fragments. Two things are tuned together and one without the other looks wrong: the falloff is a **wide, flat-cored bell**, because a heavy blur has no bright centre and that is what makes a gradient read as diffuse rather than as a shape with an edge; and the `blob` keyframes move far enough (±11 to 15% plus a 0.86 to 1.22 scale over 19 to 28s) to be visibly alive. A tighter falloff made the blobs read as hard-edged, and at that point the small original amplitude made them look frozen. **Peak alphas sit around 0.15 to 0.19 on purpose**: the project cards now carry a full-bleed accent each, so a background at the intensity this once had turns the page into noise. The background is the quiet layer; the cards are where colour belongs.
...
- `ProjectCard` — one uniform card per project; there is no featured/large variant and no tech tags. It shows only index, arrow, category, name and tagline, so it stays short. **The hover is the colour**: one `-inset-8` layer of four overlapping ellipse gradients in the project's accent, so the silhouette is irregular and covers the whole card rather than one corner. It sits at low opacity and surges on `group-hover`. The bottom-left ellipse is deliberately the weakest, because that is where the card's text sits, and the secondary text darkens to `offblack` on hover for the same reason. Measured contrast over the strongest accents is 7.9 to 11.0, against a 4.5 minimum. The card's `transform` is reserved for the hover lift, so any layout or entry animation belongs on the wrapper in the page, **not** on the card.
...
- **The blobs are radial gradients, never `filter: blur()`.** Blur on elements that size was the page's biggest GPU cost, and Safari on iOS rasterises it in tiles, which showed up on a real iPhone as a hard horizontal seam across the background. A radial gradient is soft by construction and costs nothing. Three details keep it from reading as a circle or a rectangle, and all three are load-bearing:
  - size it `closest-side`, so it reaches alpha 0 *inside* the box. The default (`farthest-corner`) still has colour where the box clips it, which draws visible rectangular seams;
  - end on the same RGB at alpha 0, never on `transparent`, which interpolates toward transparent black and leaves a grey fringe;
  - use several stops approximating a Gaussian falloff. A single linear ramp leaves a visible ring where the colour stops.
- **Blobs are sized down to roughly 60% below `md`.** A 50rem blob on a 390px screen covers everything and oversaturates; the responsive sizes keep the same proportion as desktop. Blobs marked `soloEscritorio` are `hidden md:block`.
- **Section titles use `clamp()`, never a bare `text-5xl md:text-7xl`.** "Photography" at the old fixed 48px was 346px wide inside a 310px card and spilled out of the glass on a phone. Measure the *text* with a Range, not the `h1`'s `scrollWidth`: the heading is a block, so its scrollWidth reports the container width and hides the overflow.
- **The staggered `.reveal` entry is off below `md`.** It animates `transform` on cards that carry `backdrop-filter`, so every frame forces the blurred backdrop to be recomputed; on a phone that reads as the cards loading in one by one.
...
- **Never put `overflow-x: hidden` on `html`/`body` as an overflow safety net.** It breaks `position: fixed` descendants in Safari on iOS (the fixed background stops covering the page and leaves a seam), and it silently masks real overflow so the measurement below becomes meaningless. Fix overflow where it originates instead.
